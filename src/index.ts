...
export const Config: Schema<Config> = Schema.object({
    ...
    liveDetectMode: Schema.union([
        Schema.const('WS').description('WebSocket模式：连接到对应的直播间，可推送弹幕消息，开播下播响应最快，但对订阅数有限制'),
        Schema.const('API').description('API模式：请求对应直播间API，无法获取弹幕消息，开播下播响应慢，理论可无限订阅').experimental()
    ])
        .role('radio')
        .description('直播检测模式')
        .default('WS'),
