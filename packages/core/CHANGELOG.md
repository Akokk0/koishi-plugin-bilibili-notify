...
> - ver 3.5.0 优化：改进配置说明；
> - ver 3.6.0-alpha.0 优化：改进登录操作，现在可以在配置页进行扫码登录；并移除了登录指令；
> - ver 3.6.0-alpha.1 修复：打包 `client`；
> - ver 3.6.0-alpha.2 修复：在点击登录后，切换到其他页面，再切换回来qrcode显示错误；
> - ver 3.6.0-alpha.3 优化：配置页加载个人信息附带加载动画；
> - ver 3.6.0-alpha.4 调整：将LOG和配置项改为笨蛋女仆风格、重新排列了配置项的顺序； 修复：使用高级订阅开启自定义直播总结时报错；
> - ver 3.6.0 优化：错误处理；
> - ver 3.6.1 修复：当直播下播时，恰好处于一个定时推送周期，误报直播间连接断开；
> - ver 3.6.2 修复：私聊插件错误消息不完整； 优化：增加插件配置页个人卡片加载提示；
> - ver 3.6.3 修复：打包错误；
> - ver 3.6.4 优化：添加字体；
> - ver 3.6.5 修复：一些bug； 调整：移除了自带字体；
> - ver 3.6.6 修复：开启高级订阅，打开自定义直播消息但没有填任何内容报错；
> - ver 3.7.0 重构：log、代码结构；
> - ver 3.7.1 重构：代码结构； 修复：动态推送错误、log错误；
> - ver 3.7.2 重构：代码结构； 修复：指令错误；
> - ver 3.8.0-alpha.0 新增：特定用户弹幕监听功能、特定用户进入直播间推送；
> - ver 3.8.0-alpha.1 修复：特定用户弹幕监听功能、特定用户进入直播间推送错误修复；
> - ver 3.8.0-alpha.2 修复：特定用户进入直播间不会推送的问题；
> - ver 3.8.0 优化：上游仓库更新，不再使用本地实现；
> - ver 3.9.0 优化：将SC文字推送升级为图片推送；
> - ver 3.10.0-alpha.0 功能：实现对专栏和商品的图片渲染；
> - ver 3.10.0-alpha.1 优化：特定用户进入直播间的实现暂时改回之前的方式；
...
> - ver 3.12.3 修复：某些特殊动态类型（如广告、置顶等）`pub_ts` 字段缺失导致的 `fromSeconds requires a numerical input` 崩溃；遇到无效 `pub_ts` 时记录警告日志并跳过该条动态；
